...
class ShortLogger(Callback): 
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}  #X: metrics=accuracy (not suitable for time-series)
...
#        try:
	...
	# X less neuron 100->64->32
        ...
        dum_y_vals_actual = data_normalized [ window_size : window_size + len(y_entire) ].copy()  # Fixed bugs: X- len(y_train)

                          #//To slice dum [all rows of d_n['li_close']] <- y_train (1D=(627,)) | dum's shape is 2D= (728-1-100, 5)
        dum_y_vals_actual [:, cols_to_use.index('li_close')] = y_entire  # Fixed bugs: X- len(y_train)
